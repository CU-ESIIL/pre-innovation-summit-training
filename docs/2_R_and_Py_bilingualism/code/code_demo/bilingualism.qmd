...
-   [xarray labeled multidimensional
    arrays](https://docs.xarray.dev/en/stable/) and [rioxarray arrays
    with coordinates](https://corteva.github.io/rioxarray/stable/)

-   [pandas data frames: tabular data and
    databases](https://pandas.pydata.org/docs/) and [geopandas data
    frames with coordinates](https://geopandas.org/en/stable/)
...
-   [ggplot2 data visualization (technically part of tidyverse but often
    used on its own)](https://ggplot2.tidyverse.org/) and [cowplot
    figure
...
-   [Think Python: How to Think Like a Computer
    Scientist](https://greenteapress.com/wp/think-python-2e/) by Dr.
    Allen Downey - This book is especially targeted at developing
    bilingualism, even though it is a Python book. It originally was
    published as Think Java, and then:

    <div>

    > "Jeff Elkner, a high school teacher in Virginia, adopted my book
    > and translated it into Python. He sent me a copy of his
    > translation, and I had the unusual experience of learning Python
...
-   [GitHub](https://github.com/) Issues pages for the project in
    question
...
-   Library and package names: In R, we use the randomForest package to
    ...
    RandomForestClassifier class from the sklearn.ensemble module. We
    also use different libraries for loading and manipulating data
    (pandas and numpy in Python, and built-in datasets in R). Model
    parameters: The syntax for setting model parameters is slightly
    different in R and Python. For example, in R, we specify the number
    of trees using the ntree parameter, while in Python, we use the
    n_estimators parameter. Data format: In R, we use a data frame to
    store the input data, while in Python, we use numpy arrays.
...
-   Package names and syntax: In R, we use the osmdata package and its
    ...
    access the OSM API directly, but these packages make it convenient
    to get boundaries and roads with only a few lines of code.
...
-   In both Python and R, we define the bounding box as new type of
    object (for this notebook). In R, we use a named vector; in Python,
    we use a dictionary. There are some (very) subtle differences
    between the implementation of these objects, but for now we can
    treat them as the same. Using **named** formats for data like named
    vectors, dictionaries, and dataframes help us to write clean and
    readable code. For example, in this case each coordinate could be
    either latitude or longitude and the names help distinguish that.
...
-   Function arguments: In R, function arguments are separated by
    commas, while in Python they are enclosed in parentheses. The syntax
    for specifying default arguments and variable-length argument lists
    can also differ between the two languages.

-   Return: In R, `return()` is a function that needs parentheses. In
    Python, it is a keyword, and so no parenthesed are used. This type
    of syntax in Python is called a **statement**.
...
of time), and then recombine -- all without the parallel tasks needing
to communicate with each other. There are tools, such as the [Message
...
-   Pivoting - Changing between long (or **tidy**) and wide data
    formats. Tidy data contains only a single observation in each row.
    Different statistical models require different data formats.
...
-   Both R Tidyverse and Python provide powerful tools for retrieving
    and processing data from an API. In terms of syntax, the two
    ...
    then process the JSON data. However, there are some differences in
    the specific functions and methods used. For example, in R
    Tidyverse, we use the content() function to extract the content of
    the response, whereas in Python, we use the content attribute.
    Additionally, in R Tidyverse, we use the fromJSON() function to
    convert the JSON data to a list, whereas in Python, we use the
    loads() function.
